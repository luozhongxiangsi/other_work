...
def borrow_add(request):
    ...
    for book in all_books:
        print(book)
    if request.method == 'POST':
        borrow_name = request.POST.get('borrow_name')
        bok_id = request.POST.get('book_id')
        print(borrow_name, bok_id)
        # if not borrow_name:
        #     return render(request, 'borrow_add.html', {'error': BOOK_ERROR_EMPTY, 'all_books': all_books})
        # if models.Borrow.objects.filter(borrow_name=borrow_name):
        #     return render(request, 'borrow_add.html', {'error': BOOK_ERROR_DUPLICATE, 'all_books': all_books})
        # else:
        #     pass
        models.Borrow.objects.create(borrow_name=borrow_name, book_name=models.Book.objects.get(pk=bok_id))
        return redirect('borrowlist')
    return render(request, 'borrow_add.html', {'all_books': all_books})


def give_back(request):
    nid = request.GET.get('nid')
    if type(nid) == int:
        models.Borrow.objects.filter(pk=nid).delete()
        return redirect('borrowlist')
    else:
        return redirect('borrowlist')
